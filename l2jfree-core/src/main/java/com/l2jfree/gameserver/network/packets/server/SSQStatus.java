 ...
 * 
 */
public class SSQStatus extends L2ServerPacket
{
	...
	protected final void writeImpl()
	{
		...
		switch (_page)
		{
			case 1:
				...
				/*
				 * Scoring seems to be proportionate to a set base value, so
				 * base this on the maximum obtainable score from festivals,
				 * which is 500.
				 */
			...
			case 2:
				...
				for (int i = 0; i < 5; i++)
				{
					...
					StatsSet highScoreData =
							SevenSignsFestival.getInstance().getHighestScoreData(SevenSigns.CABAL_DUSK, i);
				...
				for (int i = 1; i < 4; i++)
				{
					...
					if (_log.isDebugEnabled())
						_log.info(SevenSigns.getSealName(i, true) + " = Dawn Prop: " + dawnProportion + "("
								+ ((dawnProportion / totalDawnMembers) * 100) + "%)" + ", Dusk Prop: " + duskProportion
			...
			case 4:
				...
				for (int i = 1; i < 4; i++)
				{
					...
					int duskProportion = SevenSigns.getInstance().getSealProportion(i, SevenSigns.CABAL_DUSK);
					dawnPercent =
							Math.round((dawnProportion / (totalDawnMembers == 0 ? 1 : (float)totalDawnMembers)) * 100);
					duskPercent =
							Math.round((duskProportion / (totalDuskMembers == 0 ? 1 : (float)totalDuskMembers)) * 100);
