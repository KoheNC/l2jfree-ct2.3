	...
	public void onBypassFeedback(L2Player player, String command)
	{
		...
		else if (condition == COND_REGULAR)
		{
				...
				try
				{
					...
					if (a != null)
					{
						...
						html.replace("%AGIT_LEASE%",
								String.valueOf(ClanHallManager.getInstance().getClanHallById(a.getItemId()).getLease()));
						html.replace("%AGIT_LOCATION%", ClanHallManager.getInstance().getClanHallById(a.getItemId())
								.getLocation());
						html.replace("%AGIT_AUCTION_END%", String.valueOf(format.format(a.getEndDate())));
						html.replace(
								"%AGIT_AUCTION_REMAIN%",
								String.valueOf((a.getEndDate() - System.currentTimeMillis()) / 3600000)
										+ " hours "
						...
						html.replace("%AGIT_AUCTION_DESC%", ClanHallManager.getInstance()
								.getClanHallById(a.getItemId()).getDesc());
			...
			else if (actualCommand.equalsIgnoreCase("bid1"))
			{
				...
				if (val.isEmpty())
					return;
				if ((player.getClan().getAuctionBiddedAt() > 0 && player.getClan().getAuctionBiddedAt() != Integer
						.parseInt(val)) || player.getClan().getHasHideout() > 0)
				{
				...
				try
				{
					...
					int minimumBid =
							AuctionManager.getInstance().getAuction(Integer.parseInt(val)).getHighestBidderMaxBid();
					if (minimumBid == 0)
			...
			else if (actualCommand.equalsIgnoreCase("list"))
			{
				...
				for (int j = 1; j <= npage; j++)
					items +=
							"<td><center><a action=\"bypass -h npc_" + getObjectId() + "_list " + j + "\"> Page " + j
									+ " </a></center></td>";
				...
				for (Auction a : auctions)
				{
					...
					items +=
							"<tr>" + "<td>"
									...
									+ "</td>" + "<td><a action=\"bypass -h npc_" + getObjectId() + "_bidding "
									+ a.getId() + "\">" + a.getItemName() + "</a></td>" + "<td>"
									+ format.format(a.getEndDate()) + "</td>" + "<td>" + a.getStartingBid() + "</td>"
									+ "</tr>";
			...
			else if (actualCommand.equalsIgnoreCase("bidlist"))
			{
				...
				{
					biders +=
							"<tr>" + "<td>" + b.getClanName() + "</td><td>" + b.getName() + "</td><td>"
			...
			else if (actualCommand.equalsIgnoreCase("selectedItems"))
			{
						...
						&& player.getClan().getAuctionBiddedAt() > 0)
				{
					...
					{
						_log.warn("Auctioneer Auction null for AuctionBiddedAt : "
				...
				else if (player.getClan() != null && player.getClan().getHasHideout() != 0)
				{
					...
					if (ClanHallManager.getInstance().getClanHallById(ItemId) != null)
					{
						...
						html.replace("%AGIT_LOCATION%", ClanHallManager.getInstance().getClanHallById(ItemId)
								.getLocation());
						html.replace("%AGIT_LINK_BACK%", "bypass -h npc_" + getObjectId() + "_start");
						html.replace("%objectId%", String.valueOf(getObjectId()));
					}
					else
						_log.warn("Clan Hall ID NULL : " + ItemId
			...
			{
				int bid =
						AuctionManager.getInstance().getAuction(player.getClan().getAuctionBiddedAt()).getBidders()
								.get(player.getClanId()).getBid();
			...
			else if (actualCommand.equalsIgnoreCase("doCancelBid"))
			{
				...
				{
					AuctionManager.getInstance().getAuction(player.getClan().getAuctionBiddedAt())
