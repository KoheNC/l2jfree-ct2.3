...
public class L2Attackable extends L2Npc
{
						...
						if (attacker.getKnownList().knowsObject(this))
						{
							...
							if (!attacker.isDead())
							{
								...
								if (attacker instanceof L2PcInstance && !(this instanceof L2ChestInstance))
								{
									...
									if (addexp > 0)
										((L2PcInstance)attacker).updateVitalityPoints(getVitalityPoints(damage), true,
												false);
								}
								else
	...
	public int getHating(L2Character target)
	{
		...
		if (target instanceof L2PcInstance
				&& (((L2PcInstance)target).getAppearance().isInvisible() || target.isInvul()))
		{
	 ...
	 *            chance)
	 */
	private RewardItem calculateCategorizedRewardItem(L2PcInstance lastAttacker, L2DropCategory categoryDrops,
			int levelModifier)
	{
