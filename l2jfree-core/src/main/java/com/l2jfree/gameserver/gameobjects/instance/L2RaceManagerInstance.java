...
public class L2RaceManagerInstance extends L2Npc
{
	...
	public L2RaceManagerInstance(int objectId, L2NpcTemplate template)
	{
		...
		if (_notInitialized)
		{
			...
			s.scheduleGeneralAtFixedRate(new Announcement(SystemMessageId.MONSRACE_TICKETS_STOP_IN_S1_MINUTES),
					6 * MINUTE, 10 * MINUTE);
			s.scheduleGeneralAtFixedRate(new Announcement(SystemMessageId.MONSRACE_S1_TICKET_SALES_CLOSED), 7 * MINUTE,
					10 * MINUTE);
			s.scheduleGeneralAtFixedRate(new Announcement(SystemMessageId.MONSRACE_S2_BEGINS_IN_S1_MINUTES),
					7 * MINUTE, 10 * MINUTE);
			s.scheduleGeneralAtFixedRate(new Announcement(SystemMessageId.MONSRACE_S2_BEGINS_IN_S1_MINUTES),
					8 * MINUTE, 10 * MINUTE);
			s.scheduleGeneralAtFixedRate(new Announcement(SystemMessageId.MONSRACE_S1_BEGINS_IN_30_SECONDS), 8 * MINUTE
					+ 30 * SECOND, 10 * MINUTE);
			s.scheduleGeneralAtFixedRate(new Announcement(SystemMessageId.MONSRACE_S1_COUNTDOWN_IN_FIVE_SECONDS), 8
					* MINUTE + 50 * SECOND, 10 * MINUTE);
			s.scheduleGeneralAtFixedRate(new Announcement(SystemMessageId.MONSRACE_BEGINS_IN_S1_SECONDS), 8 * MINUTE
					+ 55 * SECOND, 10 * MINUTE);
			s.scheduleGeneralAtFixedRate(new Announcement(SystemMessageId.MONSRACE_BEGINS_IN_S1_SECONDS), 8 * MINUTE
					+ 56 * SECOND, 10 * MINUTE);
			s.scheduleGeneralAtFixedRate(new Announcement(SystemMessageId.MONSRACE_BEGINS_IN_S1_SECONDS), 8 * MINUTE
	...
	protected void broadcast(L2ServerPacket pkt)
	{
	...
	private void startRace()
	{
		...
		if (_state == STARTING_RACE)
		{
			...
			broadcast(SRace);
			PlaySound SRace2 = new PlaySound(0, this, 0, "ItemSound2.race_start");
			broadcast(SRace2);
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		if (command.startsWith("BuyTicket"))
		{
			...
			if ((val == 10 && player.getRace(0) == 0)
					|| (val == 20 && player.getRace(0) == 0 && player.getRace(1) == 0))
				val = 0;
	...
	class RunRace implements Runnable
	{
		...
		{
			_packet =
					new MonRaceInfo(_codes[2][0], _codes[2][1], MonsterRace.getInstance().getMonsters(), MonsterRace
