...
public final class L2FestivalGuideInstance extends L2Npc
{
		...
		else if (command.startsWith("Festival"))
		{
			...
			switch (val)
			{
				...
				case 3: // Score Registration
					...
					L2ItemInstance bloodOfferings =
							player.getInventory().getItemByItemId(SevenSignsFestival.FESTIVAL_OFFERING_ID);
					...
					boolean isHighestScore =
							SevenSignsFestival.getInstance().setFinalScore(player, _festivalOracle, _festivalType,
									offeringScore);
				...
				case 4: // Current High Scores
					...
					final StatsSet duskData =
							SevenSignsFestival.getInstance().getHighestScoreData(SevenSigns.CABAL_DUSK, _festivalType);
					final StatsSet overallData =
							SevenSignsFestival.getInstance().getOverallHighestScoreData(_festivalType);
					...
					if (dawnScore > 0)
						strBuffer.append("Dawn: " + calculateDate(dawnData.getString("date")) + ". Score " + dawnScore
								+ "<br>" + dawnData.getString("members") + "<br>");
					else
					...
					if (duskScore > 0)
						strBuffer.append("Dusk: " + calculateDate(duskData.getString("date")) + ". Score " + duskScore
								+ "<br>" + duskData.getString("members") + "<br>");
					else
					...
					if (overallScore > 0)
					{
						...
						if (overallData != null)
						{
							...
							strBuffer.append("Consecutive top scores: " + calculateDate(overallData.getString("date"))
									+ ". Score " + overallScore + "<br>Affilated side: " + cabalStr + "<br>"
	...
	private final String getStatsTable()
	{
		...
		for (int i = 0; i < 5; i++)
		{
			...
			tableHtml.append("<tr><td width=\"100\" align=\"center\">" + festivalName
					+ "</td><td align=\"center\" width=\"35\">" + duskScore + "</td><td align=\"center\" width=\"35\">"
	...
	private final String getBonusTable()
	{
		...
		for (int i = 0; i < 5; i++)
		{
			...
			tableHtml.append("<tr><td align=\"center\" width=\"150\">" + festivalName
