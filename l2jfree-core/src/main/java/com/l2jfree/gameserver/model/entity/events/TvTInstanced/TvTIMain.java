...
public class TvTIMain
{
	...
	{
		if (_eventTitle.equals("") || _joinLocName.equals("") || _npcX == 0 || _npcY == 0 || _npcZ == 0
				|| _instances.size() < 1)
			return false;
	...
	private static void spawnEventNpc(L2PcInstance activeChar)
	{
		...
		try
		{
			...
			_npcSpawn.getLastSpawn().broadcastPacket(
					new MagicSkillUse(_npcSpawn.getLastSpawn(), _npcSpawn.getLastSpawn(), Config.TVTI_JOIN_NPC_SKILL,
							1, 1, 1));
	...
	private static void joinNpcSkillTask()
	{
		ThreadPoolManager.getInstance().scheduleGeneral(new Runnable() {
			...
			public void run()
			{
				...
				{
					_npcSpawn.getLastSpawn().broadcastPacket(
							new MagicSkillUse(_npcSpawn.getLastSpawn(), _npcSpawn.getLastSpawn(),
									Config.TVTI_JOIN_NPC_SKILL, 1, 1, 1));
					joinNpcSkillTask();
				}
			}
	...
	public static void showEventHtml(L2PcInstance eventPlayer, String objectId)
	{
		try
		{
			if ((eventPlayer.getWeightPenalty() >= 3 || eventPlayer.getInventoryLimit() * 0.8 <= eventPlayer
					.getInventory().getSize()) && Config.TVTI_CHECK_WEIGHT_AND_INVENTORY)
			{
			...
			for (TVTInstance i : getInstances())
			{
				...
				{
					replyMSG.append("<tr></tr>");
					replyMSG.append("<td><button value=\"Remove Me!\" action=\"bypass -h npc_"
							+ objectId
							+ "_tvti_player_leave\" width=100 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
					replyMSG.append("<tr></tr>");
					replyMSG.append("<td><font color=\"FFFFFF\">Instance Name: " + i.getInstanceName() + "</font></td>");
					replyMSG.append("<tr></tr>");
					replyMSG.append("<td><font color=\"FFFFFF\">Players registered: " + i.getPlayers().size() + " / "
							+ i.getMaxPlayers() + "</font></td>");
					replyMSG.append("<tr></tr>");
					replyMSG.append("<td><font color=\"FFFFFF\">Levels: " + i.getMinLvl() + " - " + i.getMaxLvl()
							+ "</font></td>");
					break;
				}
				else if (!isPlayerInList(eventPlayer))
				{
					...
					if (eventPlayer.isCursedWeaponEquipped() && !Config.TVTI_JOIN_CURSED)
						...
						replyMSG.append("<td><button value=\"Show Instances\" action=\"bypass -h npc_"
								+ objectId
								+ "_tvti_player_join_page\" width=100 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
					break;
				}
			}
		...
		try
		{
			...
			for (TVTInstance i : getInstances())
			{
				...
				// replyMSG.append("<td><font color=\"LEVEL\">This TvT instance is not setup.</font></td>");
					...
					replyMSG.append("<td align=\"center\"><button value=\"Join\" action=\"bypass -h npc_" + objectId
							+ "_tvti_player_join " + i.getInstanceId()
				...
				replyMSG.append("<td><font color=\"FFFFFF\">Levels: " + i.getMinLvl() + " - " + i.getMaxLvl()
						+ "</font></td>");
				replyMSG.append("<tr></tr>");
				replyMSG.append("<td><font color=\"FFFFFF\">Prize: (" + i.getRewardAmount() + ")"
						+ ItemTable.getInstance().getTemplate(i.getRewardId()).getName() + "</font></td>");
				replyMSG.append("<tr></tr>");
				replyMSG.append("<td align=\"center\"><font color=\"LEVEL\">---------------------------------</font></td>");
			}
			replyMSG.append("</tr></table>");
			...
			public void run()
			{
				for (TVTInstance i : _instances)
					for (TvTITeam t : i.getTeams())
						...
						{
							summon.doRevive();
							summon.teleToLocation(summon.getOwner().getX(), summon.getOwner().getY(), summon.getOwner()
									.getZ(), false);
						}
			}
		}, Config.TVTI_REVIVE_DELAY);
	...
	public static void duplicateInstance(int instanceId)
	{
		...
		for (TvTITeam team : tempI.getTeams())
			newI.createTeam(team.getTeamName(), team.getTeamColor(), team.getSpawnX(), team.getSpawnY(),
					team.getSpawnZ(), team.getSpawnRadius());
	}
