...
public class L2Spawn
{
	...
	*/
	...
	public L2Npc doSpawn(boolean isSummonSpawn, boolean firstspawn)
	{
		...
		try
		{
			...
			if (_template.isAssignableTo(L2PetInstance.class) || _template.isAssignableTo(L2MinionInstance.class)
					|| _template.isAssignableTo(L2DecoyInstance.class)
					|| _template.isAssignableTo(L2TrapInstance.class)
	...
	private L2Npc intializeNpcInstance(L2Npc mob, boolean firstspawn)
	{
		...
		newlocy = getLocy();
		newlocz =
				doCorrect ? GeoData.getInstance().getSpawnHeight(newlocx, newlocy, getLocz(), getLocz(), _id)
						: getLocz();
		...
		if (_log.isDebugEnabled())
			_log.debug("spawned Mob ID: " + _template.getNpcId() + " ,at: " + mob.getX() + " x, " + mob.getY() + " y, "
					+ mob.getZ() + " z");
