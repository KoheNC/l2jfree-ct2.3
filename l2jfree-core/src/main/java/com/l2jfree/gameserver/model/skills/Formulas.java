...
import com.l2jfree.gameserver.model.skills.conditions.ConditionPlayerState;
import com.l2jfree.gameserver.model.skills.conditions.ConditionUsingItemType;
...
public final class Formulas
{
	...
	public static final double calcSiegeRegenModifer(L2Player activeChar)
	{
		...
		if (siegeClan == null || siegeClan.getFlag().isEmpty()
				|| !Util.checkIfInRange(200, activeChar, siegeClan.getFlag().getFirst(), true))
			return 0;
			...
			boolean crit, boolean ss)
	{
		...
		if (crit)
		{
			...
			damage *=
					2 * attacker.calcStat(Stats.CRITICAL_DAMAGE, 1, target, skill)
							* target.calcStat(Stats.CRIT_VULN, target.getTemplate().getBaseCritVuln(), target, skill);
			...
			byte shld)
	{
		...
		{
			if (calcMagicSuccess(owner, target, skill)
					&& getMagicLevelDifference(attacker.getOwner(), target, skill) >= -9)
			{
	...
	public static byte calcShldUse(L2Creature attacker, L2Creature target, L2Skill skill, boolean sendSysMsg)
	{
		...
		byte shldSuccess =
				Rnd.calcChance(Config.ALT_PERFECT_SHLD_BLOCK, 100) ? SHIELD_DEFENSE_PERFECT_BLOCK
			...
			L2Skill skill, byte shld, boolean ss, boolean sps, boolean bss)
	{
		...
		if (skill.isMagic())
			rate *=
					Math.pow((double)attacker.getMAtk(target, skill)
							/ (target.getMDef(attacker, skill) + (shld == 1 ? target.getShldDef() : 0)), 0.1);
	...
	public static boolean calcCubicSkillSuccess(L2CubicInstance attacker, L2Creature target, L2Skill skill, byte shld)
	{
		...
		if (skill.isMagic())
			rate +=
					Math.pow((double)attacker.getMAtk()
							/ (target.getMDef(attacker.getOwner(), skill) + (shld == 1 ? target.getShldDef() : 0)), 0.1) * 100 - 100;
	...
	public static byte calcSkillReflect(L2Creature target, L2Skill skill)
	{
		...
		final double reflectChance =
				target.calcStat(skill.isMagic() ? Stats.REFLECT_SKILL_MAGIC : Stats.REFLECT_SKILL_PHYSIC, 0, null,
						skill);
