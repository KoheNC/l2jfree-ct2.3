...
public class ShapeCylinder extends Shape
{
	...
	{
		return L2Math.isDistanceLessThanSq(_x, _y, x, y, _radiusSq);
	}
	...
	public boolean intersectsRectangle(int ax1, int ax2, int ay1, int ay2)
	{
		...
		if ((L2Math.pow(ax1 - _x, 2) + L2Math.pow(ay1 - _y, 2)) < _radiusSq)
			return true;
		if ((L2Math.pow(ax1 - _x, 2) + L2Math.pow(ay2 - _y, 2)) < _radiusSq)
			return true;
		if ((L2Math.pow(ax2 - _x, 2) + L2Math.pow(ay1 - _y, 2)) < _radiusSq)
			return true;
		if ((L2Math.pow(ax2 - _x, 2) + L2Math.pow(ay2 - _y, 2)) < _radiusSq)
			return true;
	...
	{
		return L2Math.calculateDistance(_x, _y, x, y) - _radius;
	}
