...
public class AdminEditNpc implements IAdminCommandHandler
{
	...
	private static final String[] ADMIN_COMMANDS = { "admin_edit_npc", "admin_save_npc", "admin_show_droplist",
			"admin_edit_drop", "admin_add_drop", "admin_del_drop", "admin_showShop", "admin_showShopList",
			"admin_addShopItem", "admin_delShopItem", "admin_editShopItem",
			"admin_close_window",
			// L2JFREE
	...
	private void delShopItem(L2PcInstance activeChar, String[] args)
	{
		...
		if (args.length > 3)
		{
			...
			activeChar.sendMessage("Deleted " + ItemTable.getInstance().getTemplate(itemID).getName()
	...
	private void editCustomShopItem(L2PcInstance activeChar, String[] args)
	{
		...
		replyMSG.append("</table>");
		replyMSG.append("<center><br><br><br>");
		replyMSG.append("<button value=\"Save\" action=\"bypass -h admin_editCustomShopItem " + tradeListID + " "
				+ itemID + " $price\"  width=100 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
	...
	private void delCustomShopItem(L2PcInstance activeChar, String[] args)
	{
		...
		replyMSG.append("</table>");
		replyMSG.append("<center><br><br><br>");
		replyMSG.append("<button value=\"Confirm\" action=\"bypass -h admin_delCustomShopItem " + tradeListID + " "
				+ itemID + " 1\"  width=100 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
		replyMSG.append("<br><button value=\"Back\" action=\"bypass -h admin_showCustomShopList " + tradeListID
				+ " 1\"  width=100 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
		replyMSG.append("</center>");
		replyMSG.append("</body></html>");
	...
	private TextBuilder customItemListHtml(L2TradeList tradeList, int page)
	{
		...
		for (L2ItemInstance item : tradeList.getItems(start, end + 1))
		{
			replyMSG.append("<tr><td><a action=\"bypass -h admin_editCustomShopItem " + tradeList.getListId() + " "
					+ item.getItemId() + "\">" + item.getItem().getName() + "</a></td>");
			replyMSG.append("<td>" + item.getPriceToSell() + "</td>");
			replyMSG.append("<td><button value=\"Del\" action=\"bypass -h admin_delCustomShopItem "
		...
		int max = tradeList.getItems().size() / PAGE_LIMIT + 1;
		if (page > 1)
			replyMSG.append("<td><button value=\"Page" + (page - 1) + "\" action=\"bypass -h admin_showCustomShopList "
					+ tradeList.getListId() + " " + (page - 1)
					+ "\" width=40 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
		if (page < max)
		{
	...
	private void showCustomShop(L2PcInstance activeChar, int merchantID)
	{
		...
		{
			if (tradeList != null)
				replyMSG.append("<tr><td><a action=\"bypass -h admin_showCustomShopList " + tradeList.getListId()
						+ " 1\">Trade List " + tradeList.getListId() + "</a></td></tr>");
		}
	...
	private void save_npc_property(L2PcInstance activeChar, String command)
	{
		...
		try
		{
			...
			{
				int intValue = Integer.parseInt(value);
				newNpcData.set("absorb_type", intValue == 0 ? "LAST_HIT" : intValue == 1 ? "FULL_PARTY"
						: "PARTY_ONE_RANDOM");
			}
		}
		catch (Exception e)
		...
		if (npcData.getDropData() != null)
		{
			for (L2DropCategory cat : npcData.getDropData())
			{
				...
				for (L2DropData drop : cat.getAllDrops())
				{
					if (drop == null)
						continue;
					replyMSG.append("<tr><td><a action=\"bypass -h admin_edit_drop " + npcData.getNpcId() + " "
							+ drop.getItemId() + " " + cat.getCategoryType() + "\">" + npcData.getNpcId() + " "
							+ drop.getItemId() + " " + cat.getCategoryType() + "</a></td>" + "<td>"
							+ ItemTable.getInstance().getTemplate(drop.getItemId()).getName() + "[" + drop.getItemId()
							+ "]" + "</td><td>" + (drop.isQuestDrop() ? "Q" : (cat.isSweep() ? "S" : "D"))
							+ "</td><td>" + "<a action=\"bypass -h admin_del_drop " + npcData.getNpcId() + " "
							+ drop.getItemId() + " " + cat.getCategoryType() + "\">del</a></td></tr>");
				}
			}
		}
	...
	private void showEditDropData(L2PcInstance activeChar, int npcId, int itemId, int category)
	{
		...
		try
		{
			...
			if (dropData.next())
			{
				...
				replyMSG.append("<center>");
				replyMSG.append("<button value=\"Save Modify\" action=\"bypass -h admin_edit_drop "
						+ npcId
						+ " "
						+ itemId
						+ " "
	...
	private void showAddDropData(L2PcInstance activeChar, L2NpcTemplate npcData)
	{
		...
		replyMSG.append("<center>");
		replyMSG.append("<button value=\"SAVE\" action=\"bypass -h admin_add_drop "
				+ npcData.getNpcId()
	...
	private void showNpcSkillList(L2PcInstance activeChar, int npcId, int page)
	{
		...
		for (int x = 0; x < MaxPages; x++)
		{
			...
			{
				pages +=
						"<td><a action=\"bypass -h admin_show_skilllist_npc " + npcData.getNpcId() + " " + x
								+ "\">Page " + pagenr + "</a></td>";
			}
		}
		...
		while (skillite.hasNext())
		{
			...
			skillobj = skillite.next();
			replyMSG.append("<tr><td><a action=\"bypass -h admin_edit_skill_npc " + npcData.getNpcId() + " "
					+ skills.get(skillobj).getId() + "\">" + skills.get(skillobj).getName() + "&nbsp;["
					+ skills.get(skillobj).getId() + "]" + "</a></td>" + "<td>" + skills.get(skillobj).getLevel()
					+ "</td>" + "<td><a action=\"bypass -h admin_del_skill_npc " + npcData.getNpcId() + " " + skillobj
					+ "\">Delete</a></td></tr>");
	...
	private void showNpcSkillEdit(L2PcInstance activeChar, int npcId, int skillId)
	{
		...
		try
		{
			...
			StringBuffer replyMSG =
					new StringBuffer("<html><title>(NPC:" + npcId + "&nbsp;SKILL:" + skillId + ")</title>");
			replyMSG.append("<body>");
			...
			if (skillData.next())
			{
				...
				replyMSG.append("<center>");
				replyMSG.append("<button value=\"Edit Skill\" action=\"bypass -h admin_edit_skill_npc " + npcId + " "
						+ skillId
						+ " $level\"  width=100 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
				replyMSG.append("<br><button value=\"Back to Skillist\" action=\"bypass -h admin_show_skilllist_npc "
						+ npcId + "\"  width=100 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
				replyMSG.append("</center>");
			}
	...
	private void updateNpcSkillData(L2PcInstance activeChar, int npcId, int skillId, int level)
	{
		...
		try
		{
			...
			if (skillData == null)
			{
				...
				replyMSG.append("<body>");
				replyMSG.append("<center><button value=\"Back to Skillist\" action=\"bypass -h admin_show_skilllist_npc "
						+ npcId
	...
	private void showNpcSkillAdd(L2PcInstance activeChar, L2NpcTemplate npcData)
	{
		...
		StringBuffer replyMSG =
				new StringBuffer("<html><title>Add Skill to " + npcData.getName() + "(ID:" + npcData.getNpcId()
						+ ")</title>");
		replyMSG.append("<body>");
		replyMSG.append("<table>");
	...
	private void addNpcSkillData(L2PcInstance activeChar, int npcId, int skillId, int level)
	{
		...
		try
		{
			...
			NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
			StringBuffer replyMSG =
					new StringBuffer("<html><title>Add Skill to Npc (" + npcId + ", " + skillId + ", " + level
							+ ")</title>");
			replyMSG.append("<body>");
			replyMSG.append("<center><button value=\"Add Skill\" action=\"bypass -h admin_add_skill_npc " + npcId
					+ "\" width=100 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
			replyMSG.append("<br><br><button value=\"Back to Skillist\" action=\"bypass -h admin_show_skilllist_npc "
					+ npcId + "\" width=100 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
			replyMSG.append("</center></body></html>");
	...
	private void deleteNpcSkillData(L2PcInstance activeChar, int npcId, int skillId)
	{
		...
		try
		{
			...
			if (npcId > 0)
			{
				...
				NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
				StringBuffer replyMSG =
						new StringBuffer("<html><title>Delete Skill (" + npcId + ", " + skillId + ")</title>");
				replyMSG.append("<body>");
				replyMSG.append("<center><button value=\"Back to Skillist\" action=\"bypass -h admin_show_skilllist_npc "
						+ npcId
						+ "\" width=100 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></center>");
