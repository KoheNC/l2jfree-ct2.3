...
public class SevenSigns
{
	...
	private SevenSigns()
	{
		...
		synchronized (this)
		{
			...
			_log.info("SevenSigns: Next period begins in " + numDays + " days, " + numHours + " hours and " + numMins
					+ " mins.");
		}
		spawnSevenSignsNPC();
	}
	...
	public long addPlayerStoneContrib(L2PcInstance player, long blueCount, long greenCount, long redCount)
	{
		...
		long totalAncientAdena =
				currPlayer.getLong("ancient_adena_amount") + calcAncientAdenaReward(blueCount, greenCount, redCount);
	...
	public boolean checkSummonConditions(L2PcInstance activeChar)
	{
		...
		if (isSealValidationPeriod())
		{
			...
			{
				activeChar
						.sendMessage("You cannot summon Siege Golem or Cannon while the Seal of Strife is possessed by the Lords of Dawn.");
				return true;
			}
		}
		return false;
