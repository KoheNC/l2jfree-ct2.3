...
public class FeedableBeasts extends L2AttackableAIScript
{
	...
	private static final int[] FEEDABLE_BEASTS = { 21451, 21452, 21453, 21454, 21455, 21456, 21457, 21458, 21459,
			21460, 21461, 21462, 21463, 21464, 21465, 21466, 21467, 21468, 21469, 21470, 21471, 21472, 21473, 21474,
	...
	private static final String[][] TEXT = {
			{ "What did you just do to me?", "You want to tame me, huh?",
					...
					"You don't need to worry about me.", "Delicious food, thanks.", "I am starting to like you!",
					"Gulp" },
	...
	private static final String[] TAMED_TEXT = { "Refills! Yeah!",
		...
		if (contains(TAMED_BEASTS, nextNpcId))
		{
			...
			L2TamedBeastInstance nextNpc =
					new L2TamedBeastInstance(IdFactory.getInstance().getNextId(), template, player,
							FOODSKILL.get(food), npc.getX(), npc.getY(), npc.getZ());
			nextNpc.setRunning();
			...
			if (rand == 0)
			{
				npc.broadcastPacket(new NpcSay(nextNpc, player.getName() + ", will you show me your hideaway?"));
			}
			else if (rand == 1)
			{
				npc.broadcastPacket(new NpcSay(nextNpc, player.getName()
			...
			{
				npc.broadcastPacket(new NpcSay(nextNpc, "Thanks, " + player.getName() + ".  I hope I can help you"));
			}
			else if (rand == 4)
			{
				npc.broadcastPacket(new NpcSay(nextNpc, player.getName() + ", what can I do to help you?"));
			}
		}
		else
		{
	...
	public String onSkillSee(L2Npc npc, L2PcInstance caster, L2Skill skill, L2Object[] targets, boolean isPet)
	{
		...
		if (_GrowthCapableMobs.containsKey(npcId))
		{
			...
			if (Rnd.get(20) == 0)
			{
				npc.broadcastPacket(new NpcSay(npc, TEXT[growthLevel][Rnd.get(TEXT[growthLevel].length)]));
			}
		...
		else if (contains(TAMED_BEASTS, npcId) && npc instanceof L2TamedBeastInstance)
		{
			...
			{
				beast.onReceiveFood();
				beast.broadcastPacket(new NpcSay(npc, TAMED_TEXT[Rnd.get(TAMED_TEXT.length)]));
			}
		}
		return super.onSkillSee(npc, caster, skill, targets, isPet);
	}
